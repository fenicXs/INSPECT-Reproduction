...
def run(config):
    ...
    # Training workflow
    if config.ckpt and config.ckpt == "test_only":
        # Load checkpoint from env var, run inference only (no training)
        import os
        ckpt_path = os.environ.get("INSPECT_CKPT_PATH", None)
        if ckpt_path is None:
            raise ValueError("ckpt=test_only requires INSPECT_CKPT_PATH env var to be set")
        print(f"Test-only mode: loading checkpoint from {ckpt_path}")
        trainer.test(model=model, datamodule=dm, ckpt_path=ckpt_path)
    elif config.ckpt and config.ckpt.endswith(".ckpt"):
