...
class DatasetBase(Dataset):
    def __init__(self, cfg, split="train", transform=None):
        ...
        path = "/scratch/pkrish52/INSPECT/data/image_pipeline/dict_slice_thickness.pkl"
        if os.path.exists(path):
            self.dict_slice_thickness = pickle.load(open(path, "rb"))
        else:
            print(f"Warning: slice thickness dict not found at {path}, using default=1.0")
    ...
    def read_image(self, file_path: str, resize_size=None, channels=None, slice_idx=0):
        ...
        if file_path.endswith('.nii.gz'):
            ...
            if len(pixel_array.shape) == 3:
                ...
                pixel_array = pixel_array[:, :, slice_idx, 0]
        else:
            # Read DICOM
            if "rsna" in self.cfg.dataset.csv_path:
                dcm = pydicom.dcmread(file_path)
