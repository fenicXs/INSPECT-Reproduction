...
class Dataset2D(DatasetBase):
    def __init__(self, cfg, split="train", transform=None):
        ...
        # get all slices for each nifti file
        self.all_instances = []
        for idx, row in tqdm.tqdm(self.df.iterrows(), total=len(self.df)):
            nifti_path = os.path.join(self.cfg.dataset.dicom_dir, row['image_id'])
            try:
                import nibabel as nib
                img = nib.load(nifti_path)
                num_slices = img.shape[2] if len(img.shape) >= 3 else 1
                for slice_idx in range(num_slices):
                    self.all_instances.append([row["impression_id"], slice_idx, nifti_path])
            except Exception as e:
                print(f"Error loading {nifti_path}: {e}")
                continue
        print(f"Total slices to featurize: {len(self.all_instances)}")
