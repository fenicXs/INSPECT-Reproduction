...
with open(os.path.join(args.path_to_data, "cohort_0.2.0_master_file_anon.csv")) as f:
    reader = csv.DictReader(f)
    for row in reader:
        pid = int(row["PatientID"])
        pid_split_assignment[pid] = row["split"]
        time = datetime.datetime.fromisoformat(row["StudyTime"])
...
def load_data(name, task, split):
    ...
    # Deduplicate by (pid, time) — keep first occurrence
    seen = set()
    dedup_indices = []
    for i, (pid, time) in enumerate(zip(result[1], result[3])):
        key = (int(pid), time)
    ...
    result = [a[dedup_indices] for a in result]
